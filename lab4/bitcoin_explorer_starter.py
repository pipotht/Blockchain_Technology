...
def task1():
    ...
    # bits = 0xEEMMMMMM : exponent EE (1 byte), mantissa MMMMMM (3 bytes)
    # target = mantissa * 256^(exponent-3)
    # Gợi ý / hint:  exponent = bits >> 24 ;  mantissa = bits & 0xFFFFFF
...
def task2(blk_hash: str):
    ...
    # fee   = sum of input prevout values  -  sum of output values   (satoshi)
    #         (input values: tx["vin"][i]["prevout"]["value"], outputs: tx["vout"][i]["value"])
    # vsize = ceil(weight / 4)   with weight = tx["weight"]           (BIP-141)
    fee = (
    sum(vin["prevout"]["value"] for vin in tx["vin"])
